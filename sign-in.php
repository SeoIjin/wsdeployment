...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT id, password, usertype, first_name, last_name, account_status FROM account WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($id, $stored_password, $usertype, $first_name, $last_name, $account_status);
    $stmt->fetch();
